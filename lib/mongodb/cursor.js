...
Cursor.prototype.fetchAllRecords = function(callback) {
  ...
  if(self.state == Cursor.INIT) {
    ...
    if(self.cursorId.greaterThan(self.db.bson_serializer.Long.fromInt(0))) {
      ...
      var getMoreCommand = new GetMoreCommand(self.db, self.db.databaseName + "." + self.collection.collectionName, self.limitValue, self.cursorId);
      // Execute the command
      self.db.executeCommand(getMoreCommand, function(err, result) {
        ...
        if(self.numberOfReturned > 0 && (self.limitValue == 0 || self.totalNumberOfRecords < self.limitValue)) {
          ...
          if(self.cursorId.greaterThan(self.db.bson_serializer.Long.fromInt(0))) self.close(function(cursor) {});
